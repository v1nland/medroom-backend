...
func DeleteEvaluacion(id string) (err error) {
	...
	if err := config.DB.Debug().
		Exec(`DELETE FROM public.calificaciones_estudiantes WHERE calificaciones_estudiantes.id_evaluacion = ?`, id).
		Error; err != nil {
		return err
	}
	...
	// delete evaluaciones
	if err := config.DB.Debug().
		Exec(`DELETE FROM public.evaluaciones WHERE evaluaciones.id = ?`, id).
		Error; err != nil {
		return err
	}
	return nil
...
func GetEvaluacionesRendidasEstudiante(u *[]models.Evaluacion, id_estudiante string) (err error) {
	if err = config.DB.Session(&gorm.Session{FullSaveAssociations: true}).
		...
		Select("ev.*").
		Table("estudiantes e").
		Joins("JOIN calificaciones_estudiantes ce ON ce.id_estudiante = e.id").
		Joins("JOIN evaluaciones ev ON ce.id_evaluacion = ev.id").
